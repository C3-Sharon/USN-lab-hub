...
- 登录响应中的 `attendance` 是对现有客户端的兼容字段，不作为个人工作台持续展示的数据源
- 登录成功后，前端必须请求 `GET /api/workbench/overview`；该接口返回的 `attendance` 是首页考勤的权威数据
- 首页刷新、签到或签退成功后，前端重新请求工作台聚合接口，不得长期使用登录时缓存的考勤数据
...
| state | 说明 | 显示方式 |
|---|---|---|
| `READY` | 模块已开放且数据加载成功 | 展示真实数据 |
| `NOT_AVAILABLE` | 模块尚未开放（业务未实现） | 显示"模块尚未开放"占位卡片 |
| `ERROR` | 模块已开放但本次调用失败 | 显示错误提示 + 重试按钮 |

`state` 是区域可用性和调用结果的唯一状态字段。不得再增加或依赖 `available` 字段；调用失败使用 `ERROR` 状态 + `errorCode`/`message` 表示。
...
本周不新增头像上传或头像 API 字段。前端使用姓名首字生成稳定的默认头像占位；姓名为空时回退使用学号首字，并保证不同页面采用同一规则。
...
| 日期 | 变更 | 状态 |
|---|---|---|
| 2026-09-14 | 冻结五档角色、权限矩阵、鉴权 Header、401/403、首页聚合、状态三态与兼容数据源 | FROZEN（产品、后端确认，可进入实现） |
